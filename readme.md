...
In case you specify a `maxlength` constraint for your slug field in the Mongoose schema, the slugger plugin will ensure that the generated slugs do not exceed the given length.

You can customize how slugs are generated by supplying your own `GeneratorFunction`. The function receives three arguments: The document, the current attempt (starting with zero), and the maximum allowed length:

```javascript
const sluggerOptions = new slugger.SluggerOptions({
  generateFrom: (doc, attempt, maxlength) => {
    // your own implementation; return the slug string
  },
  // …
});
```
